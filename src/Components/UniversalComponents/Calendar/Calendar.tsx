    ...
    const monthList = () => {
        ...
        const dayListNode = arr.map((item: ShowMonthType, index: number) => {
            ...
            const after = new Array(item.haveDay).fill(1).map((_: number, index: number) => {
                return {
                    day: _ + index,
                    currentTime: Times.currentTime(new Date(`${item.year}/${item.month}/${_ + index}`).getTime()),
                }
            })
                    ...
                    <ul className={styles.calenderUl}>
                        {_.list?.map((item: any, index: number) => {
                            return (
                                <li
                                    key={index}
                                    className={getSelect === item.currentTime ? styles.select : ``}
                                    onClick={() => setSelect(item.currentTime)}>
                                    {item.day}
                                </li>
                            )
                        })}
